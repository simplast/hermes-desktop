...
// How often the Sessions tab and in-chat sidebar re-sync from state.db while
// visible, so background sessions surface without a navigation round-trip.
...
function SessionListPanel({
  ...
  useEffect(() => {
    ...
    void refreshSessions();
  }, [currentSessionId, refreshSessions, visible]);

  useEffect(() => {
    if (!visible || searchFocusToken == null) return;
    searchRef.current?.focus();
  }, [searchFocusToken, visible]);
  ...
  return (
    <div
      className={`sessions-container ${
        variant === "sidebar" ? "sessions-container--sidebar" : ""
      }`}
    >
